          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-bold text-sm text-gray-700">Optimized Başlık</h3>
              ...
              <div className="text-xs text-gray-400 mt-1">{currentResult.title?.length || 0} karakter</div>
            </div>
            ...
            {currentResult.bullets && currentResult.bullets.length > 0 && (
              <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
                <div className="flex justify-between items-center mb-3">
                  <h3 className="font-bold text-sm text-gray-700">Bullet Points</h3>
                  <button onClick={() => copyToClipboard(currentResult.bullets.join('\n'), 'bullets')} className={`text-xs px-3 py-1 rounded-lg transition ${copied === 'bullets' ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
                    {copied === 'bullets' ? 'Kopyalandı!' : 'Tümünü Kopyala'}
                  </button>
                </div>
                <ul className="space-y-2">
                  {currentResult.bullets.map((b, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm">
                      ...
                      <button onClick={() => copyToClipboard(b, `bullet-${i}`)} className={`text-xs px-2 py-0.5 rounded transition shrink-0 ${copied === `bullet-${i}` ? 'text-green-600' : 'text-gray-400 hover:text-gray-600'}`}>
                        {copied === `bullet-${i}` ? '✓' : '⧉'}
                      </button>
                    </li>
                  ))}
            ...
            {currentResult.description && (
              <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
                <div className="flex justify-between items-center mb-2">
                  <h3 className="font-bold text-sm text-gray-700">Ürün Açıklaması</h3>
            ...
            {currentResult.seo_tips && currentResult.seo_tips.length > 0 && (
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl border border-green-100 p-5">
                <h3 className="font-bold text-sm text-green-700 mb-3">SEO İpuçları</h3>
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
        <h2 className="text-xl font-bold">📋 Listing Geçmişi</h2>
        {listings.length > 0 && (
          <button onClick={exportCSV} className="text-xs px-4 py-2 bg-white border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50 transition font-semibold">
            📥 CSV İndir ({filteredListings.length})
          </button>
        )}
      </div>
      ...
      {listings.length === 0 ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center shadow-sm">
          ...
          <h3 className="font-bold">Sonuç bulunamadı</h3>
      ...
      {filteredListings.length > 0 && (
        <div className="text-center text-xs text-gray-400 mt-4">
          Toplam {filteredListings.length} listing
        </div>
      )}
    </div>
  )
